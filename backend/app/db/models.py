...
class User(Base):
    ...
    spice_level = Column(Integer, nullable=True)  # 1-5 매운맛 수준
    adventure = Column(Enum(AdventureLevel), nullable=True)
    korean_experience = Column(Enum(KoreanExperience), nullable=True)
...
class MarketInfo(Base):
    ...
    market_info_id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    market_id = Column(String(36), ForeignKey("markets.id"), nullable=False, unique=True)
    ...
    closed_days = Column(String(200), nullable=True)  # 휴무일 (한국어) - DB 컬럼명: closed_days
    closed_days_en = Column(String(200), nullable=True)  # 휴무일 (영어) - DB 컬럼명: closed_days_en
    closed_days_zh = Column(String(200), nullable=True)  # 휴무일 (중국어) - DB 컬럼명: closed_days_zh
    closed_days_ja = Column(String(200), nullable=True)  # 휴무일 (일본어) - DB 컬럼명: closed_days_ja
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class ShopMenu(Base):
    ...
    photo_id = Column(String(36), ForeignKey("photos.id"), nullable=True)  # 가게별 메뉴 사진
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class KeywordReview(Base):
    ...
    market_id = Column(String(36), ForeignKey("markets.id"), primary_key=True)
