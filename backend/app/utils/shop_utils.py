...
def query_closest_shop(db: Session, lat: float, lng: float, max_distance_meters: float = 5000) -> Optional[str]:
    """
    ...
    Args:
        ...
        lat: 위도
        lng: 경도
        max_distance_meters: 최대 검색 거리 (미터, 기본값 5km)
    
    Returns:
        가장 가까운 가게의 shop_id (str) 또는 None (가게를 찾지 못한 경우)
    """
    try:
        ...
        if closest_shop:
            ...
            logger.info(f"최대 거리({max_distance_meters}m) 내 가게를 찾을 수 없음")
            return None
